...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 23 10:02:28 2021
...
def _get_knns(i2v, x, banned, at_k=5, reverse_picks=False, both_modes=False):
        ''' x: (1 x dim) '''
...
def covert_p1_p2(i2v, clf, active_data):
    def _process_prefix(x, p1=[], add_start=False):
        prefix = list(split_by_index(x, tokenizer.sep_token_id))[0]            
        prefix = [prefix[0]] + prefix[4:-1] + [tokenizer.eos_token_id] + [tokenizer.sep_token_id] + [p1_tok] + p1 + [tokenizer.sep_token_id]
        if add_start: prefix += [start_tok]
        return prefix
    adjusted_data = {'X':[], 'y':[]}
    # construct default personalities for agent 
    candidates = ["i don't have children.", "i don't have pets.", 
                  "i have dogs.", "i have cats.", "i love animals.",
                "i work at many jobs.", "i am looking for a new job.", 
                "i have a girlfriend.", "i am single.",
              "i have been to japan.", "i have been to europe.", 
              "i am from the united states.", "i am a middle aged man.", 
              "i am 6 feet tall.", "i am 5 foot 3.", "i am a female.", "i am a male.",
              "i am an old person.", "i am a young person.",
              "i have no tattoos.", "i have brown hair.", 
              "i am a republican.", "i am a democrat.", 
              "i like all kinds of music.", "i like blue.",
              "i like all kinds of foods.", "i like pizza.", "i like to watch comedies."]
    candidates = [c + tokenizer.eos_token for c in candidates]
    c_states = [_get_state(clf, [tokenizer.encode(c)])[0] for c in candidates]
    # searchable dict for agent p's
    c2p = dict([(k,v) for k,v in enumerate(candidates)])
    c2v = dict([(k,v) for k,v in enumerate(c_states) ])
    for i in range(len(active_data['X'])):
        x = active_data['X'][i]
        y = active_data['y'][i]
        hx_split = list(split_by_index(x, start_tok))
        if len(hx_split) == 0:
            xx = _process_prefix(x)
        else:
            start_idx = x.index(start_tok) + 1
            prefix = x[0:start_idx]
            hx = x[start_idx:]
            if act_tok in prefix:
                # filter by AGENT's turn responses
                turn_data = list(split_by_index(hx, tokenizer.eos_token_id))
                agent_hx = turn_data[0::2] + [y]
                # get state and context representation
                state, phat = _get_state(clf, agent_hx)
                # update persona input to agent
                k = min(5, len(agent_hx) )
                p1 = [c2p[p] for p in _get_knns(c2v, state.view(1,-1), [], at_k=k)]
                p1 = flatten([tokenizer.encode(p) for p in p1])
                # get entire prefix w/ action + p1
                prefix = _process_prefix(prefix, p1=p1, add_start=True)
            else:
                prefix = [p2_tok] + prefix[9:]
            xx = prefix+hx 
        adjusted_data['X'].append(xx); adjusted_data['y'].append(y)
                
    return adjusted_data
    
def single_persona_fine_tuning(i2p, old_data, single_data= None, stats = None,#max_iters = 100,
...
def pretrain(new_data, old_data, stats = None, num_train_epochs = 1):
    ...
    optimizer_grouped_parameters = [{"params": fast_group, 'lr': 5e-4}, #5e-4
                                    ...
                                    {"params": slow_group, 'lr': 1e-6}, # 1e-8
                                    {"params": normal_group, 'lr': opts.lr}] #5e-6
    optimizer = AdamW(optimizer_grouped_parameters, lr=opts.lr, eps=opts.eps)
    scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=opts.warmup_steps, 
                                                num_training_steps=t_total)
    # loading optimizer settings
