...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 29 09:36:44 2020

V6.0 action tweeking
    - 10 descriptions vs. personas vs. questions
    - turn alignment, who goes first?
    - 1 policy? 
...
class ConvGym(object):
    ...
    def interact(self, i2p, action_seq, persona_input, top_k=10, top_p=.92):
        self.agent = UserSim(i2p, None, reverse=True, top_k=top_k, top_p = top_p)
        if persona_input is not None: 
            self.user = UserSim(self.i2p, persona_input, top_k = top_k, top_p = top_p)
        else: self.user = None
        msg = None
        action_log = []
        # run for turns, updating rewards at each turn
        for turn in range(self.length):
            ...
            if action_seq is not None:
                ...
                self.agent.p1 = [i2p[p] for p in action_seq[turn]]
            else:
                ...
                self.agent.p1 = new_ps
            #self.agent.p1 = [self.i2p[p] for p in persona_input]
            msg = self.agent(msg)
            action_log.append(self.agent.p1)
            # get action
